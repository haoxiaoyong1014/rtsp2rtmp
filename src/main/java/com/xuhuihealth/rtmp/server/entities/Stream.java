...
import com.xuhuihealth.rtmp.amf.AMF0;
import com.xuhuihealth.rtmp.server.cfg.MyLiveConfig;
import com.xuhuihealth.rtmp.server.rtmp.Constants;
import com.xuhuihealth.rtmp.server.rtmp.messages.*;
...
//			System.out.println("视频输出");
	...
	/**
	 * 将连接从订阅者列表中移除，移除后订阅者列表为空返回true，否则返回false
	 * @param channel
	 */
	public synchronized boolean removeHttpFlvSubscriber(Channel channel){

		if (httpFLvSubscribers.contains(channel)){
			if (httpFLvSubscribers.size() == 1){
				sendEofToAllSubscriberAndClose();
				return true;
			}else {
				httpFLvSubscribers.remove(channel);
				channel.writeAndFlush(DefaultLastHttpContent.EMPTY_LAST_CONTENT).addListener(ChannelFutureListener.CLOSE);
			}
		}
		return false;
	}
